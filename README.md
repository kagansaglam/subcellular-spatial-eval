...
Scripts: seurat_to_csv.R and 1run_seurat_to_anndata.sh
...
Scripts: cellsp_1import_data.py to cellsp_8report_creation.py

Methodological Fix: Addressed strict 64-dimension limit errors (ValueError: setting an array element with a sequence) inherent to sparse Xenium outputs during UMAP generation. A custom "nuking" function (nuke_array) was developed to forcefully flatten nested structures into continuous 2D float32 arrays, successfully bypassing algorithmic limitations and enabling accurate manifold learning.
