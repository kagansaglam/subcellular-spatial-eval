...
sprawl_methods = [x.capitalize() for x in (args.methods[0] if isinstance(args.methods[0], list) else args.methods)]
nthreads = args.threads

print(f"====== Running SPRAWL for sample {sample} and cell type {cellType}... ======")
...
adata_st = cellSP.ch.run_sprawl(adata_st, methods=sprawl_methods, threads=nthreads)

## Write anndata object
print(f"Writing anndata with SPRAWL results for sample {sample} and cell type {cellType}")
